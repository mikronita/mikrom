...
mikrom-scheduler/  Tonic gRPC scheduler (port 5002)
mikrom-agent/      Tonic gRPC agent (port 5003), manages Firecracker VMs
mikrom-proto/      Shared protobuf definitions (agent + scheduler services)
...
# Run mikrom-scheduler (port 5002)
cargo run -p mikrom-scheduler

# Run mikrom-agent (port 5003)
...
`docker-compose up -d postgres` in `mikrom-api/` starts the PostgreSQL dependency (user: `mikrom`, password: `mikrom_password`, db: `mikrom_api`).
...
**mikrom-api** exposes three REST routes: `GET /health`, `POST /auth/register`, `POST /auth/login`, and `POST /deploy`. The `/deploy` handler connects directly to the scheduler over gRPC on every request (no persistent channel is maintained). Auth uses bcrypt + JWT (`jsonwebtoken`). Database access follows a repository trait pattern (`UserRepository`) so that `mockall` mocks can be swapped in for unit tests; integration tests spin up a real PostgreSQL container via `testcontainers`.

**mikrom-scheduler** maintains an in-memory `WorkerRegistry` (keyed by `host_id`) and an `AppScheduler` (keyed by `job_id`). On `DeployApp` it picks the highest-scoring available worker using `HostMetrics::calculate_score` with a cap of `MAX_APPS_PER_HOST = 10`, then forwards a `StartVm` RPC to that agent.

**mikrom-agent** starts a background task on startup that: (1) registers itself with the scheduler via `RegisterWorker`, then (2) loops every 5 seconds collecting `sysinfo` metrics and calling `ReportMetrics`. The gRPC server implements `AgentService` (start/stop/status VM, get metrics). `FirecrackerManager` is currently an in-memory state machine — it does not yet invoke the Firecracker binary.

**mikrom-proto** contains the `.proto` files and their pre-compiled `.rs` outputs. When `.proto` files change, regenerate with `cargo build` (the `build.rs` in the crate handles `tonic-build`). TLS cert generation/loading lives in `mikrom-proto::tls`.
...
- `AppState` (`mikrom-api/src/lib.rs`) — Axum shared state holding `PgPool` and an optional `SchedulerClient`.
