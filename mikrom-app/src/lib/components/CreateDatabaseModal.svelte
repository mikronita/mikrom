<script lang="ts">
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    const selectedPlan = plans.find((entry) => entry.id === plan);
...
<Modal
  ...
  description="Provision a managed PostgreSQL instance."
  width="max-w-[450px]"
  onclose={close}
>
              ...
              <div class="flex items-center gap-3">
                ...
                <div
                  class={`flex size-4 items-center justify-center rounded-full border ${
                    plan === p.id ? "border-primary" : "border-muted-foreground"
                  }`}
                >
                  {#if plan === p.id}
