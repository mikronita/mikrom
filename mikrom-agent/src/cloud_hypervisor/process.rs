...
impl CloudHypervisorProcess {
    pub async fn spawn(
        ...
        let child = cmd.spawn().map_err(|e| HypervisorError::ProcessError(
            format!("Failed to spawn Cloud Hypervisor: {e}"),
        ))?;

        let pid = child.id().ok_or_else(|| HypervisorError::ProcessError(
            "Failed to get CH PID".to_string(),
        ))?;
    ...
    pub async fn kill(&mut self) -> Result<(), HypervisorError> {
        ...
        self.child.kill().await.map_err(|e| HypervisorError::ProcessError(
            format!("Failed to kill CH process: {e}"),
        ))
    }
}
